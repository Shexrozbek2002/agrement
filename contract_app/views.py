    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if not serializer.data.get('applicant_phone').isdigit():
                return Response({"error": "can not enter minuse number or letter to applicant_phone"},
                                status=status.HTTP_400_BAD_REQUEST)
            # check square_of_services is not minuse number
            if serializer.data.get('square_of_services') < 0:
                return Response({"error": "can not enter minuse number to square_of_services"},
                                status=status.HTTP_400_BAD_REQUEST)
            ...
            if TypeService.objects.filter(id=serializer.data.get('service_type')).exists():
                ...
                if serializer.data.get('square_of_services') > 0:
                    if type.id == type2.type.id:
                        ...
                        if serializer.data.get('square_of_services') > 50 and serializer.data.get(
                                'square_of_services') <= 100:
                            count = serializer.data.get('square_of_services') * TwoTypeService.objects.get(id=2).value
                        if serializer.data.get('square_of_services') > 100 and serializer.data.get(
                                'square_of_services') <= 500:
                            count = serializer.data.get('square_of_services') * TwoTypeService.objects.get(id=3).value
                        if serializer.data.get('square_of_services') > 500 and serializer.data.get(
                                'square_of_services') <= 1000:
                    ...
                    return Response({"error": "You entered minuse number ", "success": False},
                                    status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response({"error": "servece_type is not existent ", "success": False},
                                status=status.HTTP_400_BAD_REQUEST)
...
class PdfVersionView(View):
    def get(self, request, *args, **kwargs):
        ...
        if contract.service_type.id == type2.type.id:
            ...
            if 50 < contract.square_of_services <= 100:
                summa = TwoTypeService.objects.get(id=2).value
            if 100 < contract.square_of_services <= 500:
                summa = TwoTypeService.objects.get(id=3).value
            if 500 < contract.square_of_services <= 1000:
                summa = TwoTypeService.objects.get(id=4).value
            if contract.square_of_services > 1000:
                summa = TwoTypeService.objects.get(id=5).value

        # if this id is not available, return not found detail
        Detail = OfertaDetail.objects.all().first()

        # information for pdf
        qr_url = '{}/agrement/aferta/{}/pdf/'.format(settings.MAIN_URL, contract.code_number)
